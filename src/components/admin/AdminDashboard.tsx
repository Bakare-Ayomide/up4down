...
import { AdSnippetManager } from "./AdSnippetManager";
import { EmailConfigManager } from "./EmailConfigManager";
import { LaunchSettingsManager } from "./LaunchSettingsManager";
...
export const AdminDashboard = ({ onLogout }: AdminDashboardProps) => {
  ...
  const handleFormSuccess = () => { setShowForm(false); setEditingItem(null); setRefreshKey((prev) => prev + 1); };
  const handleEdit = (item: any) => { setEditingItem(item); setShowForm(true); };
  const handleCancel = () => { setShowForm(false); setEditingItem(null); };
        ...
        return !showForm ? (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div><h2 className="text-2xl font-bold">Manage Downloads</h2><p className="text-muted-foreground">Add, edit, and manage your download items</p></div>
              <Button onClick={() => setShowForm(true)} className="gap-2 shadow-lg"><Plus className="h-4 w-4" /> Add New Item</Button>
            </div>
            <AdminItemList key={refreshKey} onEdit={handleEdit} />
          </div>
        ) : (
          <div className="max-w-3xl mx-auto"><AdminItemForm item={editingItem} onSuccess={handleFormSuccess} onCancel={handleCancel} /></div>
        );
      case "users": return <UserManager />;
      case "subscriptions": return <SubscriptionManager />;
      case "calendar": return <SubscriptionCalendar />;
      case "categories": return <CategoryManager />;
      case "news": return <NewsManager />;
      case "ads": return <AdManager />;
      case "ad-snippets": return <AdSnippetManager />;
      case "announcements": return <AnnouncementManager />;
      case "waitlist": return <WaitlistManager />;
      case "faq": return <FAQManager />;
      case "media": return <MediaAssetsManager />;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/30">
      <nav className="lg:hidden border-b bg-card/80 backdrop-blur-md sticky top-0 z-50">
        ...
        {mobileMenuOpen && (
          <div className="border-t border-border p-2 max-h-[60vh] overflow-y-auto">
      ...
      <div className="flex">
        ...
        <main className="flex-1 min-w-0 w-full p-3 sm:p-4 md:p-6 lg:p-8 max-w-6xl overflow-x-hidden">{renderContent()}</main>
      </div>
    </div>
  );
};
